...
class dataset(data.Dataset):
    def __init__(self, d1_root,d1_sr, d2_root, d2_sr,t2_root,t2_sr,nf_root, nf_sr,data_file, train=True):
    ...
    def __getitem__(self, index):
        ...
        d1_sobel_path=self.d1_sr+self.d1_name
        d2_sobel_path =self.d2_sr + self.d2_name
        t2_sobel_path = self.t2_sr + self.t2_name
        nonfs_sobel_path = self.nf_sr + self.nf_name

        d1_tr = torch.load(d1_path)
        d1_tr = self.norm(d1_tr)
        d1_sobel_tr=torch.load(d1_sobel_path)
        d1_sobel_tr=self.norm(d1_sobel_tr)
        
        d2_tr = torch.load(d2_path)
        d2_tr = self.norm(d2_tr)
        d2_sobel_tr = torch.load(d2_sobel_path)
        d2_sobel_tr = self.norm(d2_sobel_tr)

        t2_tr = torch.load(t2_path)
        t2_tr = self.norm(t2_tr)
        t2_sobel_tr = torch.load(t2_sobel_path)
        t2_sobel_tr =self.norm(t2_sobel_tr) 
        
        nf_tr = torch.load(nf_path)
        nf_tr = self.norm(nf_tr)
        nonfs_sobel_tr = torch.load(nonfs_sobel_path)
        nonfs_sobel_tr = self.norm(nonfs_sobel_tr)
         
        volume_tr = torch.zeros((8, 64, 64, 64), dtype=torch.float32)
        volume_tr[0] = d1_tr
        volume_tr[1] = d1_sobel_tr
        volume_tr[2] = d2_tr
        volume_tr[3] = d2_sobel_tr
        volume_tr[4] = t2_tr
        volume_tr[5] = t2_sobel_tr
        volume_tr[6] = nf_tr
        volume_tr[7] = nonfs_sobel_tr
